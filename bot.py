...
WAITING_EXAM_POINTS = set()
ACTIVE_EXAM_SESSION: Dict[int, Dict[str, str]] = {}
ACTIVE_QUESTION_SESSION: Dict[int, Dict] = {}
...
BTN_ADMIN_COURSE_STATS = "📊 إحصاءات الدورات"
BTN_ADMIN_MANAGE_EXAMS = "📝 إدارة الاختبارات"
BTN_EXAM_ADD_QUESTION = "➕ إضافة سؤال"
BTN_EXAM_FINISH_SETUP = "✅ إنهاء الاختبار"
BTN_EXAM_BACK = "⬅️ رجوع"
BTN_EXAM_ADD_ANSWER = "➕ إضافة جواب"
BTN_EXAM_CANCEL_QUESTION = "❌ إلغاء السؤال"
BTN_EXAM_ASSIGN_POINTS = "🎯 تعيين نقاط لهذا الجواب"
BTN_EXAM_FINISH_QUESTION = "✅ إنهاء السؤال"
BTN_EXAM_CANCEL_QUESTION_BACK = "⬅️ إلغاء السؤال"
BTN_EXAM_CANCEL = "❌ إلغاء الاختبار"
...
def _reset_exam_build_state(user_id: int):
...
def _delete_exam(course_id: str, exam_id: str):
    ref = _course_ref(course_id)
    if not ref:
        return
    exam_ref = ref.collection("exams").document(exam_id)
    try:
        questions = list(exam_ref.collection("questions").stream())
        for q in questions:
            q.reference.delete()
        exam_ref.delete()
    except Exception as e:
        logger.error(f"خطأ في حذف مسودة الاختبار: {e}")
...
def add_exam_to_course(course_id: str, name: str, description: str, questions: List[Dict]):
    ...
    for idx, q in enumerate(questions):
        ...
        if isinstance(options, list) and options and isinstance(options[0], dict):
            normalized_answers = [
                {"text": a.get("text", ""), "points": int(a.get("points", 0))}
                for a in options
            ]
            options = [a.get("text", "") for a in normalized_answers]
            points = [a.get("points", 0) for a in normalized_answers]
        else:
            normalized_answers = [
                {"text": opt, "points": int(points[idx]) if idx < len(points) else 0}
...
def create_exam_in_course(course_id: str, name: str) -> str:
    ...
    if not course:
        raise RuntimeError("Firestore غير متاح")
    exam_id = str(uuid.uuid4())
    course.collection("exams").document(exam_id).set(
        {"name": name, "created_at": datetime.now(timezone.utc).isoformat()}
    )
    return exam_id


def add_question_to_exam(course_id: str, exam_id: str, question_text: str, answers: List[Dict]):
    course = _course_ref(course_id)
    if not course:
        raise RuntimeError("Firestore غير متاح")
    exam_ref = course.collection("exams").document(exam_id)
    existing_questions = list_questions(course_id, exam_id)
    order = len(existing_questions)
    normalized_answers = [
        {"text": ans.get("text", ""), "points": int(ans.get("points", 0))}
        for ans in answers
    ]
    options = [a.get("text", "") for a in normalized_answers]
    points = [a.get("points", 0) for a in normalized_answers]
...
def record_exam_answer(course_id: str, exam_id: str, question_id: str, question_text: str, option_text: str, earned_points: int, user: User):
    ...
    if not ref:
        return
    exam_data = get_exam_data(course_id, exam_id)
    exam_name = (exam_data or {}).get("name")
    answer_record = {
...
def get_exam_data(course_id: str, exam_id: str):
    ref = _course_ref(course_id)
    if not ref:
        return None
    try:
        ...
        logger.error(f"خطأ في قراءة بيانات الاختبار: {e}")
        return None
...
def _list_course_participants(course_id: str):
    ref = _course_ref(course_id)
    if not ref:
        return []
    try:
...
def _list_student_courses(user_id: int):
    courses = list_courses()
    owned = []
    for c in courses:
        c_id = getattr(c, "id", None) or c.id
        if get_course_participant(c_id, user_id):
            owned.append(c)
    return owned


def _format_admin_student_stats(course_id: str, student_id: int, participant: Dict) -> str:
    course_data = get_course_data(course_id) or {}
    course_name = course_data.get("name", "دورة")
    user_record = get_user_record_by_id(student_id) or {}
    attended_days = participant.get("attended_days", [])
    answers = participant.get("answers", [])
    exam_totals: Dict[str, Dict[str, int]] = {}
    exam_names: Dict[str, str] = {}
    participated_exams = set()
    for ans in answers:
        exam_id = ans.get("exam_id", "")
        participated_exams.add(exam_id)
        earned = int(ans.get("earned_points", 0))
        agg = exam_totals.setdefault(exam_id, {"points": 0, "count": 0})
        agg["points"] += earned
        agg["count"] += 1
        if exam_id and ans.get("exam_name"):
            exam_names[exam_id] = ans.get("exam_name")

    participated_exams = {eid for eid in participated_exams if eid}

    lines = [
        f"📌 إحصاءات الطالب: {user_record.get('first_name', 'مستخدم')} {user_record.get('last_name', '')}",
        f"🎓 الدورة: {course_name}",
        "",
        f"✅ عدد الدروس التي سجّل حضورها: {len(attended_days)}",
        f"🧪 عدد الاختبارات التي شارك فيها: {len(participated_exams)}",
        f"⭐ مجموع نقاط الحضور: {participant.get('attendance_points', 0)}",
        f"📝 مجموع نقاط الاختبارات: {participant.get('exam_points', 0)}",
        f"🏅 المجموع النهائي لهذه الدورة: {participant.get('total_course_points', 0)}",
    ]

    return "\n".join(lines)


def _format_course_status_label(course_data: Dict) -> str:
    status = (course_data or {}).get("status", "active")
    return {"active": "نشطة", "paused": "متوقفة", "archived": "مؤرشفة"}.get(status, status)


def _student_stats_actions_keyboard(course_id: str, student_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("📚 تفاصيل الحضور", callback_data=f"course_admin_stats_attendance_{course_id}_{student_id}")],
            [InlineKeyboardButton("🧪 تفاصيل الاختبارات", callback_data=f"course_admin_stats_exams_{course_id}_{student_id}")],
            [InlineKeyboardButton("↩️ رجوع", callback_data=f"course_admin_stats_course_{course_id}")],
        ]
    )


def _format_attendance_details(course_id: str, participant: Dict) -> str:
    course_data = get_course_data(course_id) or {}
    course_name = course_data.get("name", "دورة")
    attended_days = participant.get("attended_days", [])
    if not attended_days:
        return f"لا توجد سجلات حضور لهذه الدورة ({course_name})."

    lines = [f"📚 تفاصيل حضور الدروس – {course_name}", ""]
    for idx, day in enumerate(attended_days, 1):
        try:
            dt = datetime.fromisoformat(day)
            formatted = dt.strftime("%Y-%m-%d %H:%M")
        except Exception:
            formatted = day
        lines.append(f"• الدرس {idx}: ✅ حضر – ({formatted})")
    return "\n".join(lines)


def _format_exam_details(course_id: str, student_id: int, participant: Dict) -> str:
    course_data = get_course_data(course_id) or {}
    course_name = course_data.get("name", "دورة")
    answers = participant.get("answers", [])
    if not answers:
        return f"لا توجد مشاركات في الاختبارات لهذه الدورة ({course_name})."

    grouped: Dict[str, List[Dict]] = {}
    for ans in answers:
        grouped.setdefault(ans.get("exam_id", ""), []).append(ans)

    lines = [f"🧪 تفاصيل الاختبارات – {course_name}", ""]
    for idx, (exam_id, ans_list) in enumerate(grouped.items(), 1):
        exam_data = get_exam_data(course_id, exam_id) if exam_id else None
        exam_name = (exam_data or {}).get("name") or ans_list[0].get("exam_name") or f"اختبار {idx}"
        total_points = sum(int(a.get("earned_points", 0)) for a in ans_list)
        lines.append(f"• {exam_name}: ✔️ {total_points} نقطة / {len(ans_list)} سؤال")
        for q in ans_list:
            status_icon = "✅" if q.get("is_correct", q.get("earned_points", 0) > 0) else "❌"
            lines.append(
                "\n".join(
                    [
                        f"🔹 سؤال: {q.get('question_text', 'سؤال')}",
                        f"• جواب الطالب: {q.get('chosen_option_text', '-')}",
                        f"• الحالة: {status_icon} ({int(q.get('earned_points', 0))} نقطة)",
                    ]
                )
            )
        lines.append("")
...
def _send_course_stats_course_picker(chat_id: int, context: CallbackContext):
    ...
    if not rows or len(rows) == 1:
        context.bot.send_message(
            chat_id=chat_id,
            text="لا توجد دورات تحتوي على مشاركين حتى الآن.",
    ...
    context.bot.send_message(
        chat_id=chat_id,
        text="اختر الدورة لعرض إحصاءاتها:",
        reply_markup=InlineKeyboardMarkup(rows),
    )


def _send_course_participants_keyboard(
    chat_id: int, course_id: str, context: CallbackContext, back_callback_data: str = None
):
    participants = _list_course_participants(course_id)
    if not participants:
    ...
    for p in participants:
        uid = int(p.id)
        record = get_user_record_by_id(uid) or {}
        label_parts = [record.get("first_name") or f"ID {uid}"]
        if record.get("last_name"):
            label_parts.append(record.get("last_name"))
...
def handle_text(update: Update, context: CallbackContext):
    ...
    if user_id in WAITING_EXAM_NAME:
        ...
        course_id = WAITING_SELECTED_COURSE.get(user_id)
        if not course_id:
            msg.reply_text("لم يتم تحديد دورة للاختبار.", reply_markup=ADMIN_COURSES_KB)
            return
        try:
            exam_id = create_exam_in_course(course_id, text)
            ACTIVE_EXAM_SESSION[user_id] = {
                "course_id": course_id,
                "exam_id": exam_id,
                "exam_name": text,
            }
            ACTIVE_QUESTION_SESSION.pop(user_id, None)
            msg.reply_text(
                f"تم حفظ عنوان الاختبار: {text}\nيمكنك الآن إضافة سؤال أو إنهاء الاختبار.",
                reply_markup=exam_actions_keyboard(),
            )
        except Exception as e:
            logger.error(f"خطأ في إنشاء الاختبار: {e}")
            msg.reply_text("تعذر إنشاء الاختبار. تأكد من تفعيل Firestore.", reply_markup=ADMIN_COURSES_KB)
        return
    ...
    if user_id in WAITING_EXAM_QUESTION:
        if user_id not in ACTIVE_EXAM_SESSION:
            ...
            return
        ACTIVE_QUESTION_SESSION[user_id] = {"text": text, "answers": [], "pending_answer": None}
        WAITING_EXAM_QUESTION.discard(user_id)
    ...
    if user_id in WAITING_EXAM_ANSWER:
        ...
        if not question_session:
            WAITING_EXAM_ANSWER.discard(user_id)
            msg.reply_text("لا يوجد سؤال محدد حاليًا.", reply_markup=exam_actions_keyboard())
            return
        question_session["pending_answer"] = text
        ACTIVE_QUESTION_SESSION[user_id] = question_session
        WAITING_EXAM_ANSWER.discard(user_id)
        WAITING_EXAM_POINTS.add(user_id)
        msg.reply_text("كم نقطة لهذا الجواب؟ (أدخل رقم مثل 0 أو 1 أو 2 …)", reply_markup=CANCEL_KB)
        return
    ...
    if user_id in WAITING_EXAM_POINTS:
        if user_id not in ACTIVE_EXAM_SESSION:
            ...
            return
        question_session = ACTIVE_QUESTION_SESSION.get(user_id)
        pending_answer = (question_session or {}).get("pending_answer")
        try:
            pts = int(text)
        except ValueError:
            pts = 0
        if not question_session or pending_answer is None:
            WAITING_EXAM_POINTS.discard(user_id)
            msg.reply_text("لا يوجد جواب لتعيين نقاطه.", reply_markup=exam_actions_keyboard())
            return
        answers = question_session.get("answers", [])
        answers.append({"text": pending_answer, "points": pts})
        question_session["answers"] = answers
        question_session["pending_answer"] = None
        ACTIVE_QUESTION_SESSION[user_id] = question_session
        WAITING_EXAM_POINTS.discard(user_id)
        msg.reply_text(
            "تم حفظ الجواب ونقاطه. يمكنك إضافة جواب آخر أو سؤال جديد أو إنهاء الاختبار.",
            reply_markup=answer_actions_keyboard(),
        )
        return

    if text == BTN_EXAM_ADD_QUESTION and user_id in ACTIVE_EXAM_SESSION:
        try:
            _persist_current_question(user_id)
        except Exception as e:
            logger.error(f"خطأ في حفظ السؤال الحالي قبل البدء بجديد: {e}")
        WAITING_EXAM_QUESTION.add(user_id)
        msg.reply_text("أرسل نص السؤال:", reply_markup=CANCEL_KB)
        return

    if text == BTN_EXAM_CANCEL_QUESTION or text == BTN_EXAM_CANCEL_QUESTION_BACK:
        WAITING_EXAM_QUESTION.discard(user_id)
        WAITING_EXAM_ANSWER.discard(user_id)
        WAITING_EXAM_POINTS.discard(user_id)
        ACTIVE_QUESTION_SESSION.pop(user_id, None)
        msg.reply_text("تم إلغاء السؤال الحالي.", reply_markup=exam_actions_keyboard())
        return

    if text == BTN_EXAM_ADD_ANSWER and user_id in ACTIVE_EXAM_SESSION:
        if user_id not in ACTIVE_QUESTION_SESSION:
            msg.reply_text("ابدأ بإضافة سؤال أولًا.", reply_markup=exam_actions_keyboard())
            return
        WAITING_EXAM_ANSWER.add(user_id)
        WAITING_EXAM_POINTS.discard(user_id)
        msg.reply_text("أرسل نص الجواب:", reply_markup=CANCEL_KB)
    ...
    if text == BTN_EXAM_FINISH_SETUP and user_id in ACTIVE_EXAM_SESSION:
        try:
            ...
            logger.error(f"خطأ في حفظ السؤال قبل إنهاء الاختبار: {e}")
        msg.reply_text(
            "تم إنهاء إعداد الاختبار. يمكنك العودة لإدارة الاختبارات أو إضافة أسئلة أخرى لاحقًا.",
            reply_markup=ADMIN_COURSES_KB,
        )
        exam_session = ACTIVE_EXAM_SESSION.get(user_id)
        if exam_session:
            _send_exam_management_menu(chat_id, exam_session.get("course_id", ""), context)
        _reset_exam_build_state(user_id)
        return

    if text == BTN_EXAM_CANCEL and user_id in ACTIVE_EXAM_SESSION:
        exam_session = ACTIVE_EXAM_SESSION.get(user_id, {})
        _reset_exam_build_state(user_id)
        try:
            _delete_exam(exam_session.get("course_id", ""), exam_session.get("exam_id", ""))
        except Exception as e:
            logger.error(f"خطأ في حذف مسودة الاختبار: {e}")
        msg.reply_text("تم إلغاء الاختبار وحذف مسودته.", reply_markup=ADMIN_COURSES_KB)
        if exam_session:
            _send_exam_management_menu(chat_id, exam_session.get("course_id", ""), context)
        return
    ...
    if text == BTN_ADMIN_ACTIVE_COURSES:
        ...
        return
    if text == BTN_ADMIN_COURSE_STATS:
        _send_course_stats_course_picker(chat_id, context)
        return
...
def handle_course_callback(update: Update, context: CallbackContext):
    ...
    if data.startswith("course_manageexams_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        course_id = data.replace("course_manageexams_", "", 1)
        WAITING_SELECTED_COURSE[user.id] = course_id
        query.answer()
        _send_exam_management_menu(chat_id, course_id, context)
        return

    if data.startswith("course_exam_new_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        course_id = data.replace("course_exam_new_", "", 1)
        WAITING_SELECTED_COURSE[user.id] = course_id
        WAITING_EXAM_NAME.add(user.id)
        ACTIVE_EXAM_SESSION.pop(user.id, None)
        ACTIVE_QUESTION_SESSION.pop(user.id, None)
        query.answer()
        context.bot.send_message(
            chat_id=chat_id,
            text="أرسل عنوان الاختبار:",
            reply_markup=CANCEL_KB,
        )
        return
    ...
    if data.startswith("course_exam_manage_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        remainder = data.replace("course_exam_manage_", "", 1)
        if "_" not in remainder:
            query.answer()
            return
        course_id, exam_id = remainder.split("_", 1)
        exam_data = get_exam_data(course_id, exam_id)
        WAITING_SELECTED_COURSE[user.id] = course_id
        query.answer()
        if not exam_data:
            context.bot.send_message(chat_id=chat_id, text="لم يتم العثور على الاختبار المطلوب.")
            return
        ACTIVE_EXAM_SESSION[user.id] = {
            "course_id": course_id,
            "exam_id": exam_id,
            "exam_name": exam_data.get("name", "اختبار"),
        }
        ACTIVE_QUESTION_SESSION.pop(user.id, None)
        context.bot.send_message(
            chat_id=chat_id,
            text=f"إدارة اختبار: {exam_data.get('name', 'اختبار')}",
            reply_markup=exam_actions_keyboard(),
        )
        return

    if data.startswith("course_addexam_"):
        # توجيه قديم لإضافة اختبار، يعيد التوجيه لإدارة الاختبارات
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
    ...
    if data.startswith("course_admin_stats_back"):
        query.answer()
        context.bot.send_message(
            chat_id=chat_id, text="🎓 إدارة الدورات", reply_markup=ADMIN_COURSES_KB
        )
        return

    if data.startswith("course_admin_stats_start"):
        query.answer()
        _send_course_stats_course_picker(chat_id, context)
        return

    if data.startswith("course_admin_stats_course_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        course_id = data.replace("course_admin_stats_course_", "", 1)
        query.answer()
        _send_course_participants_keyboard(
            chat_id, course_id, context, back_callback_data="course_admin_stats_start"
        )
        return
    ...
    if data.startswith("course_stats_"):
        ...
        query.answer()
        if is_admin(user.id) or is_supervisor(user.id):
            _send_course_participants_keyboard(
                chat_id, course_id, context, back_callback_data=f"course_admin_manage_{course_id}"
            )
            return
        participant = get_course_participant(course_id, user.id)
        if not participant:
    ...
    if data.startswith("course_admin_stats_student_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        payload = data.replace("course_admin_stats_student_", "", 1)
        if "_" not in payload:
            query.answer()
            return
        course_id, student_raw = payload.split("_", 1)
        student_id = int(student_raw)
        query.answer()
        participant = get_course_participant(course_id, student_id)
        if not participant:
            context.bot.send_message(chat_id=chat_id, text="لا تتوفر بيانات للطالب في هذه الدورة.")
            return
        stats_text = _format_admin_student_stats(course_id, student_id, participant)
        context.bot.send_message(
            chat_id=chat_id,
            text=stats_text,
            reply_markup=_student_stats_actions_keyboard(course_id, student_id),
        )
        return

    if data.startswith("course_admin_stats_attendance_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        payload = data.replace("course_admin_stats_attendance_", "", 1)
        if "_" not in payload:
            query.answer()
            return
        course_id, student_raw = payload.split("_", 1)
        ...
        participant = get_course_participant(course_id, student_id)
        if not participant:
            context.bot.send_message(chat_id=chat_id, text="لا توجد بيانات حضور لهذا الطالب.")
            return
        context.bot.send_message(
            chat_id=chat_id,
            text=_format_attendance_details(course_id, participant),
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("↩️ رجوع", callback_data=f"course_admin_stats_student_{course_id}_{student_id}")]]
            ),
        )
        return

    if data.startswith("course_admin_stats_exams_"):
        if not (is_admin(user.id) or is_supervisor(user.id)):
            query.answer("غير مصرح")
            return
        payload = data.replace("course_admin_stats_exams_", "", 1)
        if "_" not in payload:
            query.answer()
            return
        course_id, student_raw = payload.split("_", 1)
        ...
        participant = get_course_participant(course_id, student_id)
        if not participant:
            context.bot.send_message(chat_id=chat_id, text="لا توجد بيانات اختبارات لهذا الطالب.")
            return
        context.bot.send_message(
            chat_id=chat_id,
            text=_format_exam_details(course_id, student_id, participant),
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("↩️ رجوع", callback_data=f"course_admin_stats_student_{course_id}_{student_id}")]]
            ),
        )
...
def _send_exam_management_menu(chat_id: int, course_id: str, context: CallbackContext):
    course = get_course_data(course_id)
    if not course:
        context.bot.send_message(chat_id=chat_id, text="تعذر العثور على الدورة.")
        return
    exams = list_exams(course_id)
    rows = [[InlineKeyboardButton("➕ إضافة اختبار جديد", callback_data=f"course_exam_new_{course_id}")]]
    for e in exams:
        e_data = e.to_dict() if hasattr(e, "to_dict") else e
        rows.append(
            [
                InlineKeyboardButton(
                    e_data.get("name", "اختبار"),
                    callback_data=f"course_exam_manage_{course_id}_{getattr(e, 'id', '')}",
                )
            ]
        )
    rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data=f"course_admin_manage_{course_id}")])
    context.bot.send_message(
        chat_id=chat_id,
        text="📝 إدارة الاختبارات لهذه الدورة:",
        reply_markup=InlineKeyboardMarkup(rows),
    )
...
def _send_admin_course_controls(chat_id: int, course_id: str, context: CallbackContext):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(BTN_ADMIN_MANAGE_EXAMS, callback_data=f"course_manageexams_{course_id}")],
