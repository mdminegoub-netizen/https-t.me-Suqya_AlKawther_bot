...
# المنطقة الزمنية الافتراضية للجرعة التحفيزية (قابلة للتعديل عبر المتغير البيئي MOTIVATION_TZ)
MOTIVATION_TZ = pytz.timezone(os.getenv("MOTIVATION_TZ", "Africa/Algiers"))
...
def _normalize_times(raw_times, fallback: List[str]) -> List[str]:
    ...
    # دعم النصوص المفصولة بأي رموز (مثل — أو ,)
    if isinstance(raw_times, str):
        raw_iterable = re.findall(r"\d{1,2}:\d{2}", raw_times)
    else:
        raw_iterable = raw_times or []

    for t in raw_iterable:
        hour = None
        minute = None
...
def motivation_job(context: CallbackContext):
    now_local = datetime.now(MOTIVATION_TZ)
    current_time_str = now_local.strftime("%H:%M")
    logger.info("Running motivation job for %s (%s)...", current_time_str, MOTIVATION_TZ.zone)
...
def handle_admin_motivation_times_start(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        ...
        f"الأوقات الحالية ({tz_label}): {current}\n\n"
...
def handle_admin_motivation_times_input(update: Update, context: CallbackContext):
    ...
    msg.reply_text(
        ...
        f"الأوقات الجديدة ({MOTIVATION_TZ.zone}): {hours_text}",
        reply_markup=ADMIN_MOTIVATION_KB,
    )
